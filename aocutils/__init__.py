# -*- coding: utf-8 -*-

from __future__ import unicode_literals


#                    _   _ _
#  __ _ ___  __ _  _| |_(_) |___
# / _` / _ \/ _| || |  _| | (_-<
# \__,_\___/\__|\_,_|\__|_|_/__/
#
# High level API for PythonOCC

__name__ = "aocutils"
__description__ = "High level API for PythonOCC "

__version__ = "0.1.0"
__author__ = "Guillaume Florent, Adam Lange, Jelle Feringa, Thomas Paviot"
__author_email__ = "florentsailing@gmail.com"
__license__ = 'GPL v3'
__url__ = "https://github.com/floatingpointstack/pythonocc-utils"
__download_url__ = "https://github.com/floatingpointstack/pythonocc-utils/releases/tag/" + __version__
